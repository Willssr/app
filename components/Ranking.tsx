...
const Ranking: React.FC<RankingProps> = ({ users }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h2 className="text-3xl font-bold text-center mb-6 text-accent">Leaderboard</h2>
      <div className="space-y-3">
        {sortedUsers.map((user, index) => (
          <div key={user.id} className="bg-background-secondary border border-border-color rounded-xl p-4 flex items-center justify-between shadow-lg transition-transform hover:scale-105 hover:border-accent/50 duration-200">
            ...
            <p className="font-bold text-lg text-accent">{user.points} pts</p>
          </div>
        ))}
      </div>
    </div>
